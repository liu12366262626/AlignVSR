...
class FANPredictor(object):
    ...
    @torch.no_grad()
    def old_call(self, image: np.ndarray, face_boxes: np.ndarray, rgb: bool = True,
    ...
    @torch.no_grad()
    def __call__(self, images: np.ndarray, face_boxes: np.ndarray, rgb: bool = True,
                 return_features: bool = False, old=False) -> Union[Tuple[np.ndarray, np.ndarray],
                                                         Tuple[np.ndarray, np.ndarray, torch.Tensor]]:
        if old:
            return self.old_call(images, face_boxes, rgb, return_features)
        if face_boxes.size > 0:
            if not rgb:
                images = images[..., ::-1]
            if face_boxes.ndim == 1:
            ...
            for image, face_box in zip(images, face_boxes):
                face_patches = []
                # Crop the faces
                centres = (face_box[:, [0, 1]] + face_box[:, [2, 3]]) / 2.0
                face_sizes = (face_box[:, [3, 2]] - face_box[:, [1, 0]]).mean(axis=1)
                enlarged_face_box_sizes = (face_sizes / self.config.crop_ratio)[:, np.newaxis].repeat(2, axis=1)
                enlarged_face_boxes = np.zeros_like(face_box[:, :4])
                enlarged_face_boxes[:, :2] = np.round(centres - enlarged_face_box_sizes / 2.0)
                enlarged_face_boxes[:, 2:] = np.round(enlarged_face_boxes[:, :2] + enlarged_face_box_sizes) + 1
                enlarged_face_boxes = enlarged_face_boxes.astype(int)
                outer_bounding_box = np.hstack((enlarged_face_boxes[:, :2].min(axis=0),
                ...
                for left, top, right, bottom in enlarged_face_boxes:
                    ...
                    face_patches.append(cv2.resize(image[top: bottom, left: right, :],
                                                (self.config.input_size, self.config.input_size)))
                face_patches_list.extend(face_patches)
                enlarged_face_boxes_list.extend(enlarged_face_boxes)
                size_list.append(len(face_patches))
            
            # pdb.set_trace()
            face_patches_list = torch.from_numpy(np.array(face_patches_list).transpose(
                (0, 3, 1, 2)).astype(np.float32)).to(self.device) / 255.0
            # Get heatmaps
            heatmaps, stem_feats, hg_feats = self.net(face_patches_list)
            ...
            # Rectify landmark coordinates
            hh, hw = heatmaps.size(2), heatmaps.size(3)
            for landmark, (left, top, right, bottom) in zip(landmarks, enlarged_face_boxes_list):
                landmark[:, 0] = landmark[:, 0] * (right - left) / hw + left
                landmark[:, 1] = landmark[:, 1] * (bottom - top) / hh + top
