    ...
    def __init__(self, subset, cfg):
        if subset == "train":
            self.video_pipeline = torch.nn.Sequential(
                ...
                torchvision.transforms.RandomCrop(cfg.crop_size),
            ...
            self.video_pipeline = torch.nn.Sequential(
                FunctionalModule(lambda x: x / 255.0),
                torchvision.transforms.CenterCrop(cfg.crop_size),
                torchvision.transforms.Grayscale(),
                torchvision.transforms.Normalize(0.421, 0.165),
            )
...
class TextTransform:
    ...
    def __init__(
        ...
        dict_path=DICT_PATH,# word map to id (腌 3)
    ):
    ...
    def tokenize(self, text):
        tokens = self.spm.EncodeAsPieces(text)# split text
        token_ids = [self.hashmap.get(token, self.hashmap["<unk>"]) for token in tokens]
        return torch.tensor(list(map(int, token_ids)))#return sequence to id
