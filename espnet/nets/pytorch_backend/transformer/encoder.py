...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        # attention position encoding way
    ...
    def forward(self, xs, masks, extract_resnet_feats=False):
        ...
        """
        if isinstance(self.frontend, (Conv1dResNet, Conv3dResNet)):#xs torch.Size([12, 82, 1, 88, 88])
            xs = self.frontend(xs)
        if extract_resnet_feats:#xs torch.Size([12, 82, 512])
            return xs
